...
public class AuctionRoomService {
        ...
        return AuctionDataStore.findById(auctionId).map(auction -> {
            try {
                ...
                if (cleanAmount.isEmpty()) {
                    return new ServiceResult<AuctionRoomViewModel>(false, "Vui lòng nhập số tiền hợp lệ.", convertToViewModel(auction));
                }
                ...
                if (!auction.validateBid(bidAmount)) {
                    ...
                    return new ServiceResult<AuctionRoomViewModel>(false, "Giá đặt phải ít nhất bằng: " + minBidStr, convertToViewModel(auction));
                }
                ...
                return new ServiceResult<>(true, "Đặt giá thành công cho phiên " + auctionId + "!", convertToViewModel(auction));
            } catch (InvalidBidException e) {
                return new ServiceResult<AuctionRoomViewModel>(false, "Số tiền đấu giá không hợp lệ.", convertToViewModel(auction));
            } catch (AuctionClosedException e) {
                return new ServiceResult<AuctionRoomViewModel>(false, "Phiên đấu giá đã kết thúc.", convertToViewModel(auction));
            }
    ...
    private AuctionRoomViewModel convertToViewModel(Auction auction) {
        String leadingBidder = (auction.getHighestBidder() == null) ? "Chưa có người dẫn đầu" : auction.getHighestBidder().getUsername();
        ...
        return new AuctionRoomViewModel(
                ...
                "Lịch phiên: " + dateTimeFormatter.format(auction.getStartTime()) + " - " + dateTimeFormatter.format(auction.getEndTime()),
                auction.getBidHistory().stream()
                        .map(t -> t.getBidder().getUsername() + " đã đặt " + currencyFormatter.format(t.getBidAmount()) + " lúc " + dateTimeFormatter.format(t.getTimestamp()))
                        .toList()
        );
    }
}

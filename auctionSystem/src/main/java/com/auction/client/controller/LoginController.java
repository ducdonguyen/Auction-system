...
public class LoginController {
    ...
    private void handleLoginAction() {
        ...
        if (result.success()) navigate("/views/AuctionList.fxml", "Hệ thống đấu giá", 1200, 760);
    }
    @FXML
    private void handleGoToRegister() { navigate("/views/Register.fxml", "Đăng ký", 980, 720); }
    private void navigate(String fxmlPath, String title, int width, int height) {
        try { SceneNavigator.switchScene(loginButton, fxmlPath, title, width, height); } catch (IOException e) { errorLabel.setText("Lỗi điều hướng."); }
    }
}
